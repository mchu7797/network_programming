...
int processReadEvent(ClientSession *session) {
  while (1) {
    ...
    int n = recv(session->sock, session->readBuffer + session->readOffset,
                 needBytes, 0);
    ...
    if (n > 0) {
      ...
      if (session->readOffset >= session->expectedBytes) {
        if (session->state == STATE_READING_HEADER) {
          // 헤더 읽기 완료
          memcpy(&session->currentHeader, session->readBuffer,
          ...
          if (session->currentHeader.body_length_ > 0) {
            ...
            session->bodyBuffer =
                (char *)malloc(session->currentHeader.body_length_);
          ...
          // 바디 읽기 완료
          memcpy(session->bodyBuffer, session->readBuffer,
                 session->currentHeader.body_length_);
          session->state = STATE_PROCESSING;
          return 1;
        }
      }
...
void handlePacket(ClientSession *session) {
  ...
  case PROTOCOL_ACTION_JOIN: {
    if (body) {
      ...
      char joinMsg[256];
      snprintf(joinMsg, sizeof(joinMsg), "%s님이 입장하셨습니다.",
               session->username);
  ...
  case PROTOCOL_ACTION_WHISPER: {
    if (body) {
      ...
      printf("[WHISPER] %s -> %s: %s\n", whisper->sender_, whisper->target_,
             message);
      ...
      if (!sendWhisper(whisper->target_, whisper->sender_, message)) {
        ...
        int totalLen =
            sizeof(struct PacketHeader) + sizeof(struct BodyChat) + msgLen;
  ...
  while (running) {
    ...
    case TASK_CLOSE: {
      ...
      if (strlen(session->username) > 0) {
        char quitMsg[256];
        snprintf(quitMsg, sizeof(quitMsg), "%s님이 퇴장하셨습니다.",
                 session->username);
